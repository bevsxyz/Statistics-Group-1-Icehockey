...
```{r}
# nullModel = lm(marker ~ 1, data=top_10 |> select(-c(Name,avg_points)))
# fullModel = lm(marker ~ ., data= top_10 |> select(-c(Name,avg_points)))
# step(
#   fullModel,
#   scope = list(lower = nullModel, upper = fullModel),
#   direction = "forward"
#  ) |>  glance()
  # fortify() |> ggplot(aes(x = .fitted, y = .stdresid)) +
  # geom_point() +
  # geom_hline(yintercept = 0) +
  # stat_smooth(geom = "smooth") +
  # labs(x = "Fitted Values", y = "Residuals", color="Country")
