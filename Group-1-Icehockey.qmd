...
plot.corr <- function(df, method){
  ...
  ggplot(df.corr(df,method), aes(var_1, var_2, fill = cor, label = round(cor, 2))) +
...
-    **shootR**: Right shooters have an estimated increase in `avg_points` over the baseline set by the left shooters, by approximately 0.05878 when other variables are held constant. However, this change is not statistically
    significant, as the
    p-value is 0.1493.
...
#| fig-width: 8
#| fig-height: 8
#| fig-cap: "Spearman Rank Correlation Matrix of all the Variables with the binary variable marker, denoting whether the player is a top performer."
