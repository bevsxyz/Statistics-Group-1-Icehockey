...
Points per game is our dependent variable. Hence, it would be prudent to avoid goals and assists the independent variables from which it is calculate.
...
We can see that there are 4 categorical variables, 11 doubles, and one time duration. We can convert time to seconds. We also will remove the variables from which we had calculated our dependent variable, i.e, no. of games played, goals, assists, and points.
...
We will now get a statistical summary of all the variables, to get an idea of our variable distribution.
...
df.corr = df |>  select(where(is.numeric)) |>  cor()
# Convert the matrix output to tibble dataframe
mrows = rownames(df.corr)
df.corr = df.corr |>
...
ggplot(df.corr, aes(var_1, var_2, fill = cor, label = round(cor, 2))) +
...
We do not have any pairs having multicollinearity. Hence, we can move ahead with our features.

We will now use smoothening to find the optimal curve in xy scatter plot of the dependent variable with each independent variable.
...
We can see that graphically all the variables except duration has approximately linear relationship with average points. In the case of duration we have an outlier that is causing the relationship to deviate from linear. We went back to the source to check the datapoint. Here we found that the outlier was due to human error.

```{r}
df |> select(Name,country, duration) |>  filter(duration == max(duration))
```

We will fix the outlier and check the linear relationship again.
