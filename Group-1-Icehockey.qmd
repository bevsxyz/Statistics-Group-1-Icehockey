...
In this section, we conduct an exploratory data analysis (EDA) to
understand the distribution and relationships within our dataset. Our
dependent variable of interest is "Points per Game" (avg_points), and we
aim to identify key factors influencing this variable.
...
The dataset contains 4 categorical variables, 11 numeric variables, and
one time duration variable. We convert the time variable to seconds and
remove variables related to the dependent variable.
...
Graphically, all variables, except duration, exhibit an approximately
linear relationship with average points. However, the presence of an
outlier in duration causes a deviation from linearity. Upon
investigation, we identify the outlier as a result of human error.

```{r}
df |> select(Name,country, duration) |>  filter(duration == max(duration))
```

To address the outlier, we correct the duration and reevaluate the linear relationship.
...
fullModel = lm(avg_points ~.-country,data=df |> select(-Name))
...
model_residual |>
  ...
  facet_wrap(country ~ shoot)
```

The residual analysis ensures that the model assumptions are met.
Additionally, we check the normality of residuals using a QQ plot.
...
The QQ plot confirms the normal distribution of residuals. The
regression analysis provides insights into the relationships between
variables and their impact on average points in women's ice hockey.
