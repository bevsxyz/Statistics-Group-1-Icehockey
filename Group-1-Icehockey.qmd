...
```{r}
df = df |> select(!c(goals_g, assists_a, points)) # Drop since they are related to the dependent variable
df = df |>  mutate(total_minutes_as_duration =
                     as.numeric(total_minutes_as_duration, unit="secs")) |> 
  rename(avg_points = points_per_game_ppg,
...
Now we will encode the categorical variables encoding for country, position, and shoot.
...
df.num = df |>  select(where(is.numeric)) # correlation can only be done on numeric
```
...
df.corr = df.corr |>  as_tibble(rownames = "var_1")|> gather(key="var_2", value="cor", -1) |> drop_na()
```
...
```{r}
df.corr = df.corr |> filter(!is.na(cor))
df.corr.chosen.num = df.corr |> filter(case_when(
  var_2 != "points_per_game_ppg" ~ abs(cor) < 0.7,
  TRUE ~ FALSE
))
```
...
```{r}
ggplot(df.corr.chosen.num,aes(var_1, var_2, fill= cor,label=round(cor,2))) +
    geom_tile() +
  geom_text(aes(colour="white")) +
  ylim(mrows) + xlim(rev(mrows)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```

```{r}
df.corr.chosen.num |> distinct(var_1)
```
