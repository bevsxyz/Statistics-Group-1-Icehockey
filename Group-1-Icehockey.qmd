...
We can see that there are 4 categorical variables, 11 doubles, and one time. We can convert time to seconds.

```{r}
df = df |> select(!c(games_played_gp,goals_g, assists_a, points)) # Drop since they are related to the dependent variable
...
# Remove the upper triangular matrix
# upperm =upper.tri(df.corr,diag = T)
# df.corr[upperm] = NA
# as_tibble(df.corr[,"avg_points"],rownames = "var") |> arrange(desc(value))
```
...
We will drop one of the independent variable pairs having absolute value of correlation above 0.7.
...
```{r}
df = df |> mutate(duration = replace(duration, Name == "Emma Ingold", as.numeric(
  as.difftime("01:30:24", format = "%H:%M:%S", units = "secs")
)))
df |> select(where(is.numeric)) |>
  gather(yvar, val, -avg_points) |>
  ggplot(aes(val, avg_points)) +
...
```{r}
df |> select(avg_points, duration, penalty_dur) |>
  #filter(duration < 1e5) |>
  gather(yvar, val,-avg_points)  |>
  ggplot(aes(val)) +
  geom_histogram() +
...
```{r}
df |> select(avg_points, duration, penalty_dur) |>
  filter(duration < 1e5 & penalty_dur < 20) |>
  gather(yvar, val, -avg_points)  |>
  ggplot(aes(val)) +
  geom_histogram() +
  facet_wrap(yvar ~ ., ncol = 2, scales = "free_x")
...
fullModel = lm(avg_points ~ .,data=df |> select(-Name))#country+position+shoot+height+penalty_dur+age+weight+sog+g_diff+duration, data=df)
model = step(fullModel,scope=list(lower=nullModel, upper=fullModel),
                  direction="both")
```

```{r}
summary(lm(avg_points ~ country+position+shoot+weight+sog+g_diff+duration, data=df))
```
