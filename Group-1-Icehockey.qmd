...
We also wished to check the semi-partial correlation to understand the unique contribution of each individual predictor variable to the dependent variable (`avg_points`),
while accounting for the effects of all other predictors. It helps
answer the question, "How much does the correlation between a specific
predictor and the dependent variable change when we control for the
other predictors in the model?" However, we faced issues calculating the same with categorical variables. Hence, we only checked the semi-partial correlation regarding the numeric variables @tbl-spcor.
